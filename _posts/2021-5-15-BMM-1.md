...
TopkBMM kernel is extremely fast! it's up to 10x faster than naive "bruteforce search" when number of data points are close to a million. To be honest I wasn't expact this. And this is not the end! we can further improve its speed by switching to half type (float16), it's also possible to use tensor cores of more recent nvidia GPUs to accelerate the matrix multiplication.
