...
```  
However, all threads still participate in loading matrix A and B from global memory. (No arithmetic ops.)  
...
Now let's see how the new MBMM kernel compares to the original custom BMM + masked_fill and torch.bmm (cuBLAS) + masked_fill:
...
### 2. batch size = 128, k = 64, varying m and n (m = n):
